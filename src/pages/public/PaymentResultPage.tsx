...
import { CheckCircle2, XCircle, Package, ArrowLeft, Home, RefreshCw, Hammer } from 'lucide-react';
...
export default function PaymentResultPage() {
  ...
  const type = searchParams.get('type');           // "custom-order" | null (regular order)
  const id = searchParams.get('id');               // custom order id
  const orderCode = searchParams.get('orderCode'); // regular order code
  ...
  useEffect(() => {
    // Auto-redirect after 6s on success
    if (isSuccess) {
      const timer = setTimeout(() => {
        if (isCustomOrder && id) {
          navigate(`/custom-orders/my/${id}`);
        } else {
          navigate('/orders');
        }
      }, 6000);
  ...
  }, [isSuccess, isCustomOrder, id, orderCode, navigate]);

  const successTitle = isCustomOrder
    ? 'Thanh toán đơn gia công thành công!'
    : 'Thanh toán thành công!';

  const successSubtitle = isCustomOrder
    ? 'Đơn gia công đã được xác nhận. Nghệ nhân sẽ bắt đầu thực hiện sớm nhất!'
    : 'Đơn hàng của bạn đã được xác nhận. Chúng tôi sẽ liên hệ sớm nhất có thể!';

  const autoRedirectHint = isCustomOrder
    ? 'Tự động chuyển đến chi tiết đơn gia công sau 6 giây...'
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-stone-100 border border-stone-100 overflow-hidden">
          ...
          <div className="p-8 text-center">
            ...
            >
              {isSuccess ? (
                isCustomOrder ? (
                  <Hammer size={52} className="text-emerald-500" />
                ) : (
            ...
            {/* Order/custom-order badge */}
            {(orderCode || (isCustomOrder && id)) && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                {isCustomOrder ? <Hammer size={14} /> : <Package size={14} />}
                {isCustomOrder ? `Đơn gia công #${id}` : orderCode}
              </motion.div>
            )}
            ...
            >
              {isSuccess ? (
                <>
                  {isCustomOrder && id ? (
                    <Link
                      to={`/custom-orders/my/${id}`}
                      id="payment-view-custom-order-btn"
